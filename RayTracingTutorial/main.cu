...
int main() {
    ...
    for (int i = 0; i < NUM_GPUS; i++) {
        ...
        checkCudaErrors(cudaMemcpyToSymbolAsync(cam, &host_cam, sizeof(camera), 0, cudaMemcpyDefault, gpus[i].stream_camera));
    ...
    for (int i = 0; i < NUM_GPUS; ++i) {
        ...
        render <<<blocks_per_grid, threads_per_block, 0, gpus[i].stream_image_buffer>>>(gpus[i].d_image_buffer, image_width, image_height, samples_per_pixel_per_GPU, max_depth, gpus[i].d_world, gpus[i].d_rand_state_render);
        checkCudaErrors(cudaGetLastError());
